...
export const makeMessageMethods = (ctx) => ({
     ...
     */
    relayMessage: async (jid, message, { messageId, statusJidList } = {}) => {
        const client = await ctx.getClient();
        const user = ctx.getUser();
        ...
        const id = jid === 'status@broadcast' && statusJidList?.length
     ...
     */
    requestPlaceholderResend: async (messageKey, _msgData) => {
        const message = {
            protocolMessage: {
