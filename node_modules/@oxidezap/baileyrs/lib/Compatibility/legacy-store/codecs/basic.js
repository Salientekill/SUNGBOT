...
import { parseUnsignedDecimal, requireByteArray, requireBytes, requireSafeUnsignedInteger, serializeSignalPublicKey } from '../validation.js';
function prekeyCodec() {
    return {
        fromLegacy(key, value) {
            ...
            return proto.PreKeyRecordStructure.encode(proto.PreKeyRecordStructure.create({ id: parseUnsignedDecimal(key, 'pre-key id'), publicKey, privateKey })).finish();
        },
        toLegacy(_key, value) {
            const record = proto.PreKeyRecordStructure.decode(value);
            if (!record.publicKey || !record.privateKey)
                throw new TypeError('native pre-key record is missing key material');
            return { public: Buffer.from(record.publicKey), private: Buffer.from(record.privateKey) };
        }
    };
}
function syncKeyCodec() {
    return {
        fromLegacy(_key, value) {
            ...
            const fingerprint = message.fingerprint
                ? proto.Message.AppStateSyncKeyFingerprint.encode(proto.Message.AppStateSyncKeyFingerprint.create(message.fingerprint)).finish()
                : new Uint8Array();
            return toJsonBytes({
        ...
        toLegacy(_key, value) {
            ...
            const fingerprint = fingerprintBytes.length > 0
...
function syncVersionCodec() {
    return {
        fromLegacy(_key, value) {
            ...
            const state = value;
            const indexValueMap = {};
            for (const [index, entry] of Object.entries(state.indexValueMap ?? {}).toSorted(([a], [b]) => a.localeCompare(b))) {
                indexValueMap[index] = bytesToNumbers(entry.valueMac);
            }
            return toJsonBytes({
                version: state.version ?? TimeValue.UNKNOWN_SECONDS,
                hash: state.hash ? bytesToNumbers(state.hash) : Array(AppState.HASH_LENGTH).fill(0),
...
function lidMappingCodec() {
    return {
        fromLegacy(key, value) {
            ...
            if (parsed.direction === LidMapping.PN_DIRECTION)
                return new TextEncoder().encode(mapping);
            return toJsonBytes({
                lid: parsed.user,
        ...
        toLegacy(key, value) {
            const parsed = parseLidMappingKey(key);
            if (parsed.direction === LidMapping.PN_DIRECTION)
                return Buffer.from(value).toString(JsonByteEncoding.UTF8);
            const native = fromJsonBytes(value);
            if (typeof native.phone_number !== 'string')
                throw new TypeError('native LID mapping has no phone number');
            return native.phone_number;
        }
    };
}
export function createBasicCodecs() {
    return {
