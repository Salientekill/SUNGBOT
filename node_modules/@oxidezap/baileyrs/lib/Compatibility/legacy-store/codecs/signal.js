...
import { serializeSignalPublicKey } from '../validation.js';
import { BufferJSON } from '../../../Utils/generics.js';
const toBase64 = (value) => Buffer.from(value).toString(JsonByteEncoding.BASE64);
...
const optionalBuffer = (value) => Buffer.from(value ?? EMPTY_BYTES);
...
function senderKeyFromLegacy(value) {
...
const messageKeyIndex = (value) => {
    if (!/^\d+$/.test(value))
        throw new TypeError('legacy skipped-message index must be an unsigned integer');
...
function messageKeysFromLegacy(messageKeys) {
...
function sessionFromLegacy(value, defaults) {
    if (value === null || typeof value !== 'object')
        throw new TypeError('legacy session record must be an object');
    const record = value;
    ...
    if (!defaults)
        throw new TypeError('session import requires local Signal credentials');
    const decoded = {
...
function coreEntryToLegacy(session) {
    const chains = {};
...
function sessionToLegacy(nativeBytes) {
    ...
    if (projection.status === 'unrepresentable')
        return NATIVE_ONLY_PROJECTION;
    const sessions = {};
...
export function createSignalCodecs(defaults) {
    return {
        [NativeStore.SESSION]: {
