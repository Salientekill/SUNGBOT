 ...
 */
const hydratedScalar = (kind, value) => {
    if (value instanceof LongRuntime)
        return value;
    const split = isObject(value) && typeof value.low === 'number' && typeof value.high === 'number';
    if (typeof value !== 'number' && typeof value !== 'string' && !split)
        return value;
...
let aliasBySchema;
const aliasFor = (schemaId) => (aliasBySchema ?? (aliasBySchema = PROTO_MESSAGE_SCHEMAS.map(([path]) => FIELD_ALIASES[path])))[schemaId];
/**
...
const repairMessage = (schemaId, value, ancestors) => {
    ...
    for (const field of fields) {
        ...
        if (converted !== current)
            (output ?? (output = { ...value }))[fieldKey] = converted;
    }
 ...
 */
const messageFieldsByNameBySchema = [];
const messageFieldsOf = (schemaId) => {
 ...
 * like `repairMessage`.
 */
const projectAliases = (schemaId, value) => {
    if (!isObject(value))
        return value;
    ...
    for (const key of Object.keys(value)) {
            ...
            if (isObject(current)) {
                ...
                for (const entry in current) {
                    ...
                    if (item !== current[entry])
                        (entries ?? (entries = { ...current }))[entry] = item;
                }
                converted = entries ?? current;
            }
        }
        ...
        if (converted !== current)
            (output ?? (output = { ...value }))[key] = converted;
    }
    return output ?? value;
};
/**
 ...
 */
export const projectProtoMessage = (path, message) => {
    const schemaId = schemaIdFor(path);
 ...
 */
const fieldValue = (data, field, alias) => !hasOwn(data, field[0]) && alias !== undefined && alias[0] === field[0] && hasOwn(data, alias[1])
    ? data[alias[1]]
    : data[field[0]];
/** True when either spelling is an own property, which is what keeps prototype defaults out of `toObject`. */
const hasOwnField = (data, field, alias) => hasOwn(data, field[0]) || (alias !== undefined && alias[0] === field[0] && hasOwn(data, alias[1]));
class ProtoCompatibilityRuntime {
    ...
    toObject(schemaId, input, options = {}) {
        ...
        for (const field of fields) {
            ...
            if (value === null || value === undefined || !hasOwnField(data, field, alias))
                continue;
    ...
    projectForEncode(schemaId, value, requireCodecs = true) {
        if (!isObject(value))
            return value;
        let output;
        ...
        if (alias && hasOwn(value, alias[0])) {
            ;
            (output ?? (output = { ...value }))[alias[1]] = value[alias[0]];
            delete output[alias[0]];
        }
