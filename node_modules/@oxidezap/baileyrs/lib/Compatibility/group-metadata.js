import { WAMessageAddressingMode } from '../Types/Message.js';
import { isLidUser, jidEncode, jidNormalizedUser } from '../WABinary/jid-utils.js';
/**
 ...
 */
const isBaileysParticipantPn = (jid) => typeof jid === 'string' && jid.endsWith('@s.whatsapp.net');
const normalizeOptionalUserJid = (jid) => (jid ? jidNormalizedUser(jid) : undefined);
const participantAdmin = (participantType) => {
    if (participantType === 'admin' || participantType === 'superadmin')
 ...
 */
export const bridgeGroupMetadataToBaileys = (group) => {
    const participants = group.participants.map(participant => ({
        id: participant.jid,
        phoneNumber: isLidUser(participant.jid) && isBaileysParticipantPn(participant.phoneNumber)
    ...
    const hasDescriptionNodeData = group.description !== undefined ||
    ...
    return {
        ...
        notify: group.notify,
        addressingMode: group.addressingMode === WAMessageAddressingMode.LID ? WAMessageAddressingMode.LID : WAMessageAddressingMode.PN,
...
//# sourceMappingURL=group-metadata.js.map