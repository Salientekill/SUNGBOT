...
import { encodeStubParticipant } from './group-stub-params.js';
import { areJidsSameUser, isLidUser, isPnUser } from '../WABinary/jid-utils.js';
/**
...
const isParticipantAction = (action) => action.type in PARTICIPANT_STUBS;
/** Match the object built from a `<participant>` by upstream messages-recv. */
export const canonicalGroupParticipantToBaileys = (participant) => {
    const admin = participant.role === 'admin' || participant.role === 'superadmin' ? participant.role : null;
    const result = { id: participant.jid, admin };
...
const validateRequestMethod = (raw) => {
    if (!raw)
        return undefined;
    return KNOWN_REQUEST_METHODS.has(raw)
        ? raw
        : undefined;
};
const sameDefinedUser = (left, right) => left !== undefined && right !== undefined && areJidsSameUser(left, right);
const participantMatchesAuthor = (participant, notification) => sameDefinedUser(participant.jid, notification.author) ||
...
export const buildGroupJoinRequestEvents = (notification) => {
...
export const buildGroupNotificationDomainEvent = (notification) => {
    const action = notification.action;
    ...
    const update = { id: notification.groupJid, ...change };
    if (notification.author !== undefined)
        update.author = notification.author;
    if (notification.authorPn !== undefined)
        update.authorPn = notification.authorPn;
    if (notification.authorUsername !== undefined)
        update.authorUsername = notification.authorUsername;
    return { name: 'groups.update', payload: [update] };
};
/** Chat updates that upstream process-message derives while handling stubs. */
export const buildGroupNotificationChatUpdates = (notification) => {
    const action = notification.action;
    switch (action.type) {
        case 'subject':
...
const participantStubRecipe = (notification, action) => {
    let stubType = PARTICIPANT_STUBS[action.type];
    ...
    return {
        stubType,
        stubParams: action.participants.map(participant => encodeStubParticipant(canonicalGroupParticipantToBaileys(participant))),
        idSuffix: action.type
    };
};
const joinRequestStubRecipes = (notification) => buildGroupJoinRequestEvents(notification).map((request, index) => {
...
const stubRecipesFor = (notification) => {
    const action = notification.action;
    if (isParticipantAction(action))
        return [participantStubRecipe(notification, action)];
...
const stubMessageId = (notification, recipe, recipeIndex, count) => {
    const actionIndex = notification.actionIndex;
    if (notification.notificationId) {
        if (actionIndex === 0 && count === 1)
...
const groupStubMessageKey = (notification, fromMe, id, participant, participantAlt) => ({
...
export const buildGroupNotificationStubMessages = (notification, fromMe) => {
    const recipes = stubRecipesFor(notification);
    return recipes.map((recipe, index) => {
        const key = groupStubMessageKey(notification, fromMe, stubMessageId(notification, recipe, index, recipes.length), notification.author, notification.authorPn);
...
export const buildGroupCreateStubMessage = (notification, metadata, fromMe) => {
    const recipe = {
    ...
    return WAProto.WebMessageInfo.create({
        key: groupStubMessageKey(notification, fromMe, stubMessageId(notification, recipe, 0, 1), metadata.owner, metadata.ownerPn),
        participant: notification.author,
        messageTimestamp: notification.timestamp,
        messageStubType: recipe.stubType,
        messageStubParameters: recipe.stubParams
...
//# sourceMappingURL=group-notifications.js.map