import { isHostedLidUser, isHostedPnUser, isLidUser, isPnUser, jidDecode, jidEncode, WAJIDDomains } from '../WABinary/index.js';
const nativeContexts = new WeakMap();
const isPnNamespace = (jid) => isPnUser(jid) || isHostedPnUser(jid);
const isLidNamespace = (jid) => isLidUser(jid) || isHostedLidUser(jid);
const lidForPnDevice = (pn, lid) => {
...
export const jidToSignalProtocolAddressCompat = (jid) => {
...
export const bindSignalRepositoryContext = (auth, ctx) => {
...
const resolveMappings = async (client, inputs, direction, ctx) => {
    if (inputs.length === 0)
        return null;
    const unique = [...new Set(inputs)].filter(jid => direction === 'pn-to-lid' ? isPnNamespace(jid) : isLidNamespace(jid));
    const settled = await Promise.allSettled(unique.map(async (input) => {
        if (direction === 'pn-to-lid') {
            const resolved = await client.lidForPn(input);
            const lid = resolved ? lidForPnDevice(input, resolved) : null;
            return lid ? { pn: input, lid } : null;
        }
        const resolved = await client.pnForLid(input);
        const pn = resolved ? pnForLidDevice(input, resolved) : null;
        return pn ? { pn, lid: input } : null;
    }));
    const mappings = [];
