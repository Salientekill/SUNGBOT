...
export const generateWAMessageContent = async (message, options) => {
    ...
    if ((hasOptionalProperty(message, 'mentions') && message.mentions?.length) ||
        ...
        if (key && typeof key === 'object') {
            const target = key;
            const ci = (target.contextInfo ?? (target.contextInfo = {}));
 ...
 * client created last.
 */
let activeBridgeClient;
let activeBridgeLogger;
export const _registerActiveBridgeClient = (client, logger) => {
 ...
 */
export const downloadContentFromMessage = async (mediaContent, type, opts = {}) => {
...
export function getAggregateVotesInPollMessage({ message, pollUpdates }, meId) {
    ...
    for (const update of pollUpdates || []) {
        ...
        for (const optionHash of vote.selectedOptions) {
            ...
            if (!aggregate)
                aggregate = voteHashMap[hash] = { name: 'Unknown', voters: [] };
...
export const aggregateMessageKeysNotFromMe = (keys) => {
    ...
    for (const { remoteJid, id, participant, fromMe } of keys) {
        ...
        keyMap[uniqueKey] || (keyMap[uniqueKey] = { jid: remoteJid, participant: participant, messageIds: [] });
