...
import { BRIDGE_SK_KEY_LID, BRIDGE_SK_KEY_PN_DEV, UPSTREAM_SK_KEY_LID, UPSTREAM_SK_KEY_PN_DEV, buildBridgeSenderKeyBytes, fill, makeWrapped } from "./_legacy-store-fixtures.js";
describe('wrap-legacy-store: sender_key key-name translation', () => {
    ...
    test('bridge GET reads back from the upstream-formatted key', async () => {
        const { wrapped, keys } = await makeWrapped();
        const upstreamBytes = Buffer.from(JSON.stringify([
            {
                senderKeyId: 17,
...
describe('wrap-legacy-store: sender_key value-byte translation', () => {
    test('bridge proto bytes load via upstream `SenderKeyRecord.deserialize`', async () => {
        ...
        const onDisk = keys.raw['sender-key']?.[UPSTREAM_SK_KEY_LID];
        ...
        expect(Buffer.from(state.getSenderChainKey().getSeed()).equals(chainSeed)).toBe(true);
        expect(Buffer.from(state.getSigningKeyPublic()).equals(signPub)).toBe(true);
        expect(Buffer.from(state.getSigningKeyPrivate()).equals(signPriv)).toBe(true);
    });
    test('upstream JSON bytes round-trip to valid bridge proto', async () => {
        ...
        keys.raw['sender-key'] = { [UPSTREAM_SK_KEY_LID]: upstreamBytes };
        const bytesOut = (await wrapped.get('sender_key', BRIDGE_SK_KEY_LID));
...
//# sourceMappingURL=wrap-legacy-store-sender-key.test.js.map