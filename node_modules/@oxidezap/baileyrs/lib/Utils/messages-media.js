...
import { getBinaryNodeChild, getBinaryNodeChildBuffer } from '../WABinary/generic-utils.js';
...
export async function getAudioDuration(buffer) {
    ...
    let musicMetadata;
    try {
        musicMetadata = await import(__rewriteRelativeImportExtension('music-metadata'));
    }
    catch {
        return undefined;
    }
...
export async function generateThumbnail(bufferOrPath, mediaType, options) {
    let thumbnail;
...
export const decryptMediaRetryData = ({ ciphertext, iv }, mediaKey, msgId) => proto.MediaRetryNotification.decode(aesDecryptGCM(ciphertext, getMediaRetryKey(mediaKey), iv, Buffer.from(msgId)));
