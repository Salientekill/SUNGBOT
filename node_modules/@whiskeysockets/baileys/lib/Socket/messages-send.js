...
import { aggregateMessageKeysNotFromMe, assertMediaContent, assertMeId, bindWaitForEvent, decryptMediaRetryData, downloadContentFromMessage, downloadMediaMessage, encodeNewsletterMessage, encodeSignedDeviceIdentity, encodeWAMessage, encryptMediaRetryRequest, extractDeviceJids, generateMessageIDV2, generateParticipantHashV2, generateWAMessage, getStatusCodeForMediaRetry, getUrlFromDirectPath, getWAUploadToServer, invalidateRecentMessageOnUpdate, MessageRetryManager, normalizeMessageContent, parseAndInjectE2ESessions, runDetached, unixTimestampSeconds } from '../Utils/index.js';
import { getUrlInfo } from '../Utils/link-preview.js';
import { makeKeyedMutex, makeMutex } from '../Utils/make-mutex.js';
import { getMessageReportingToken, shouldIncludeReportingToken } from '../Utils/reporting-utils.js';
import { commitTcTokenWithIndex, isTcTokenExpired, resolveIssuanceJid, resolveTcTokenJid, shouldSendNewTcToken, storeTcTokensFromIqResult } from '../Utils/tc-token-utils.js';
import { areJidsSameUser, isHostedLidUser, isHostedPnUser, isJidBot, isJidGroup, isJidMetaAI, isLidUser, isPnUser, jidDecode, jidEncode, jidNormalizedUser, PSA_WID, S_WHATSAPP_NET } from '../WABinary/index.js';
import { USyncQuery, USyncUser } from '../WAUSync/index.js';
import { makeNewsletterSocket } from './newsletter.js';
export const makeMessagesSocket = (config) => {
    const { logger, linkPreviewImageThumbnailWidth, generateHighQualityLinkPreview, options: httpRequestOptions, patchMessageBeforeSending, cachedGroupMetadata, enableRecentMessageCache, maxMsgRetryCount } = config;
    const sock = makeNewsletterSocket(config);
    const { ev, authState, messageMutex, signalRepository, upsertMessage, query, fetchPrivacySettings, sendNode, groupMetadata, groupToggleEphemeral, registerSocketEndHandler, refreshMediaConn, getMediaHost } = sock;
     ...
     */
    const downloadMediaMessageWithHost = (...[message, type, options, ctx]) => downloadMediaMessage(message, type, { host: getMediaHost(), ...options }, ctx);
    const downloadContentFromMessageWithHost = (message, type, opts = {}) => downloadContentFromMessage(message, type, { host: getMediaHost(), ...opts });
    return {
        ...sock,
        userDevicesCache,
        devicesMutex,
        ...
        getMediaUrl: (directPath) => getUrlFromDirectPath(directPath, getMediaHost()),
        /**
        ...
        sendMessage: async (jid, content, options = {}) => {
            ...
            if (typeof content === 'object' &&
                ...
                if (config.emitOwnEvents) {
                    ...
                    messageMutex.mutex(fullMsg.key.remoteJid ?? '__no-chat__', () => upsertMessage(fullMsg, 'append')), logger, { op: 'emitOwnEvents.upsertMessage', msgId: fullMsg.key.id }));
                }
                return fullMsg;
            }
        }
