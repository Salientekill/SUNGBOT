...
import { commitTcTokenWithIndex, isTcTokenExpired, readTcTokenIndex, resolveIssuanceJid, resolveTcTokenJid, storeTcTokensFromIqResult, TC_TOKEN_INDEX_KEY } from '../Utils/tc-token-utils.js';
...
// ============================================================================
// PROTEÇÕES ANTI-TRAVAMENTO (portadas do baileys-mod do @aleatory)
// Camada aditiva sobre o núcleo de concorrência do rc13-rebased (lock-manager,
// retry manager, messageMutex por chat). Não substitui nada — só adiciona
// dedup, filtro de mensagens antigas, throttle de retry por grupo e um
// heartbeat de diagnóstico de congelamento.
// ============================================================================
const MESSAGE_DEDUP_CONFIG = {
    ...
    // Filtrar mensagens offline antigas ao reconectar
    FILTER_OLD_OFFLINE_MESSAGES: true
};
// 🛑 Throttle de retries por grupo — evita loop de retry em grupos grandes.
const RETRY_LIMITER = {
...
function markGroupAsLarge(groupJid) {
    ...
    const t = setTimeout(() => RETRY_LIMITER.largeGroups.delete(groupJid), 10 * 60 * 1000);
    t.unref?.();
}
function canProcessRetry(groupJid, logger) {
    ...
    if (data.count > RETRY_LIMITER.MAX_RETRIES_PER_GROUP) {
        ...
        logger.warn({ group: groupJid, retryCount: data.count, cooldownMinutes: RETRY_LIMITER.COOLDOWN_MS / 60000 }, '🛑 Grupo com muitos retries — bloqueando temporariamente');
        return false;
    }
    return true;
}
// 🔍 Diagnóstico de congelamento — apenas observacional (loga, não intervém).
...
function startFreezeDetector(logger) {
    ...
    freezeCheckInterval = setInterval(() => {
        ...
        if (FREEZE_DETECTOR.currentOperation && timeSinceLastProcess > FREEZE_DETECTOR.MUTEX_TIMEOUT_MS) {
            logger.error({ operation: FREEZE_DETECTOR.currentOperation, messageId: FREEZE_DETECTOR.currentMessageId, timeSinceLastProcess: `${Math.round(timeSinceLastProcess / 1000)}s`, queueSize: FREEZE_DETECTOR.queueSize }, '🚨 POSSÍVEL CONGELAMENTO DETECTADO');
        }
        else if (timeSinceLastProcess > FREEZE_DETECTOR.PROCESSING_TIMEOUT_MS && FREEZE_DETECTOR.queueSize > 0) {
            logger.warn({ timeSinceLastProcess: `${Math.round(timeSinceLastProcess / 1000)}s`, queueSize: FREEZE_DETECTOR.queueSize, currentOp: FREEZE_DETECTOR.currentOperation || 'idle' }, '⚠️ Processamento lento detectado');
        }
    }, FREEZE_DETECTOR.HEARTBEAT_INTERVAL_MS);
    freezeCheckInterval.unref?.();
}
...
export const makeMessagesRecvSocket = (config) => {
    ...
    const retryLockRef = (msgId, participant) => ({
        namespace: 'msg-retry',
    ...
    const inFlightIdentityRefreshes = new Set();
    // ── Anti-duplicação: cache LRU de IDs já processados (por from:id) ──
    ...
    const isMessageAlreadyProcessed = (messageId, from) => processedMessageIds.has(`${from}:${messageId}`);
    const markMessageAsProcessed = (messageId, from) => {
        if (messageId && from) {
            processedMessageIds.set(`${from}:${messageId}`, Date.now());
        }
    };
    const isMessageTooOld = (timestamp) => {
        ...
        return ageMs > MESSAGE_DEDUP_CONFIG.MAX_MESSAGE_AGE_MS;
    };
    const shouldProcessOfflineMessage = (node) => {
        ...
        return !(timestamp > 0 && isMessageTooOld(timestamp));
    };
    ...
    const requestPlaceholderResend = async (messageKey, msgData) => {
        ...
        const alreadyHandled = await placeholderResendLocks.withLock({ namespace: 'placeholder-resend', id: resendId }, async () => {
    ...
    const sendRetryRequest = async (node, forceIncludeKeys = false) => {
        ...
        // 🛑 Throttle de retries por grupo — impede tempestade de retries em
        // grupos grandes (complementa o retry manager, que limita por msgId).
        if (msgKey?.remoteJid && isJidGroup(msgKey.remoteJid) && !canProcessRetry(msgKey.remoteJid, logger)) {
            logger.debug({ group: msgKey.remoteJid, msgId }, 'retry de grupo suprimido pelo throttle');
            return;
        }
    ...
    const handleMessage = async (node) => {
        const dedupId = node.attrs?.id;
        const dedupFrom = node.attrs?.from;
        const dedupTimestamp = parseInt(node.attrs?.t || '0', 10);
        const isOfflineNode = !!node.attrs?.offline;
        // ── Anti-duplicação: mensagem já processada (após reconexão) ──
        if (dedupId && dedupFrom && isMessageAlreadyProcessed(dedupId, dedupFrom)) {
            logger.debug({ messageId: dedupId, from: dedupFrom }, 'Mensagem duplicada — ACK e ignora');
            await sendMessageAck(node);
            return;
        }
        // ── Filtro de mensagens antigas (>5min) ──
        if (dedupTimestamp > 0 && isMessageTooOld(dedupTimestamp)) {
            logger.debug({ messageId: dedupId, from: dedupFrom, timestamp: dedupTimestamp }, 'Mensagem muito antiga — ACK e ignora');
            await sendMessageAck(node);
            return;
        }
        // ── Filtro de mensagens offline antigas ──
        if (isOfflineNode && !shouldProcessOfflineMessage(node)) {
            logger.debug({ messageId: dedupId, from: dedupFrom }, 'Mensagem offline antiga filtrada — ACK e ignora');
            await sendMessageAck(node);
            return;
        }
        // heartbeat de diagnóstico
        FREEZE_DETECTOR.lastProcessedTime = Date.now();
        FREEZE_DETECTOR.currentOperation = 'handleMessage';
        FREEZE_DETECTOR.currentMessageId = dedupId || '';
        const encNode = getBinaryNodeChild(node, 'enc');
        ...
        try {
            ...
            await messageMutex.mutex(messageChatKey, async () => {
                ...
                // Marca como processada só após decrypt + upsert bem-sucedidos.
                // Mensagens que falharam (retry receipt) NÃO são marcadas, para
                // que o reenvio (resposta ao retry) seja processado normalmente.
                if (msg.messageStubType !== proto.WebMessageInfo.StubType.CIPHERTEXT) {
                    markMessageAsProcessed(dedupId, dedupFrom);
                }
            });
        }
        catch (error) {
            ...
            FREEZE_DETECTOR.currentMessageId = '';
            FREEZE_DETECTOR.lastProcessedTime = Date.now();
        }
    };
