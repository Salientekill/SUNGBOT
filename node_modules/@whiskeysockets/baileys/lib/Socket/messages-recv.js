...
export const makeMessagesRecvSocket = (config) => {
    ...
    const withAck = async (node, handler) => {
        try {
            await handler();
        }
        finally {
    ...
    const handleReceipt = (node) => {
        return withAck(node, async () => {
            ...
            if (Array.isArray(content)) {
                ...
                const itemIds = items.map(i => i.attrs.id).filter(Boolean);
                ids.push(...itemIds);
            }
