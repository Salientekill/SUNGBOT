...
export const makeMessagesRecvSocket = (config) => {
    const { logger, retryRequestDelayMs, maxMsgRetryCount, getMessage, ignoreOfflineMessages, shouldIgnoreJid, enableAutoSessionRecreation } = config;
    ...
    const handleMessage = async (node) => {
        ...
        if (unavailableNode) {
            logger.warn({ id: node.attrs.id, from: node.attrs.from, type: unavailableNode.attrs?.type }, 'received unavailable message, sending positive ack');
            await sendMessageAck(node);
            return;
        }
