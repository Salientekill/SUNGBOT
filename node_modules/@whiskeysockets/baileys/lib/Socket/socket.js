...
export const makeSocket = (config) => {
    ...
    const getAvailablePreKeysOnServer = async () => {
        ...
        // Verificação defensiva: retorna 0 se countChild ou attrs.value for undefined
        // 0 forçará upload de pre-keys, que é o comportamento seguro
        if (!countChild || !countChild.attrs?.value) {
            return 0;
        }
    ...
    const requestPairingCode = async (phoneNumber, customPairingCode) => {
        // Simulate human typing delay (1.5-4 seconds)
        // Humans take time to type the phone number
        const typingDelay = gaussianDelay(2500, 800, 1500, 4000);
        logger.debug({ delay: typingDelay }, 'Simulating human pairing input delay...');
        ...
        // Small delay before sending request (500-1500ms)
        // Simulates human hesitation/verification before submitting
        const requestDelay = gaussianDelay(900, 300, 500, 1500);
        logger.debug({ delay: requestDelay }, 'Delay before sending pairing request...');
