...
export const encodeStubParticipant = (p) => {
    const out = { id: p.id };
    if (isNonEmptyString(p.phoneNumber))
        out.phoneNumber = p.phoneNumber;
    if (isNonEmptyString(p.lid))
        out.lid = p.lid;
    if (isNonEmptyString(p.username))
        out.username = p.username;
    if (p.admin === 'admin' || p.admin === 'superadmin' || p.admin === null)
        out.admin = p.admin;
 ...
 */
export const decodeStubParticipant = (raw) => {
    if (!raw)
        return null;
    let parsed;
    try {
    ...
    if (typeof obj.id !== 'string')
        return null;
    const out = { id: obj.id };
    if (isNonEmptyString(obj.phoneNumber))
        out.phoneNumber = obj.phoneNumber;
    if (isNonEmptyString(obj.lid))
        out.lid = obj.lid;
    if (isNonEmptyString(obj.username))
        out.username = obj.username;
    if (obj.admin === 'admin' || obj.admin === 'superadmin' || obj.admin === null)
        out.admin = obj.admin;
    return out;
};
//# sourceMappingURL=group-stub-params.js.map