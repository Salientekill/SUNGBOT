...
import { CanonicalTag, JsonByteEncoding, MirrorEnvelope } from './constants.js';
...
export const toJsonBytes = (value) => new TextEncoder().encode(JSON.stringify(value));
export const fromJsonBytes = (value) => JSON.parse(new TextDecoder().decode(value));
...
function canonicalize(value, seen) {
    ...
    if (typeof value === 'number') {
        if (Number.isNaN(value))
            return { [CanonicalTag.NUMBER]: 'NaN' };
        if (value === Infinity)
            return { [CanonicalTag.NUMBER]: 'Infinity' };
        if (value === -Infinity)
            return { [CanonicalTag.NUMBER]: '-Infinity' };
        if (Object.is(value, -0))
            return { [CanonicalTag.NUMBER]: '-0' };
        return value;
    }
    if (typeof value === 'bigint')
        return { [CanonicalTag.BIGINT]: value.toString() };
    if (value instanceof Uint8Array || Buffer.isBuffer(value)) {
        return { [CanonicalTag.BYTES]: Buffer.from(value).toString(JsonByteEncoding.BASE64) };
    }
    if (value instanceof Date)
        return { [CanonicalTag.DATE]: value.toISOString() };
    if (typeof value !== 'object')
        return { [CanonicalTag.UNSUPPORTED]: typeof value };
    if (seen.has(value))
 ...
 */
export function encodeNativeOnlyEnvelope(payload, legacyValue) {
 ...
 */
export function decodeNativeEnvelope(value) {
...
export function envelopeMatchesLegacy(envelope, legacyValue) {
