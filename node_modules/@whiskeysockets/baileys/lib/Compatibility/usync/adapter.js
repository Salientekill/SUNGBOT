...
import { USyncBotProfileProtocol } from '../../WAUSync/Protocols/USyncBotProfileProtocol.js';
import { USyncContactProtocol } from '../../WAUSync/Protocols/USyncContactProtocol.js';
import { USyncDeviceProtocol } from '../../WAUSync/Protocols/USyncDeviceProtocol.js';
import { USyncDisappearingModeProtocol } from '../../WAUSync/Protocols/USyncDisappearingModeProtocol.js';
import { USyncLIDProtocol } from '../../WAUSync/Protocols/USyncLIDProtocol.js';
import { USyncStatusProtocol } from '../../WAUSync/Protocols/USyncStatusProtocol.js';
import { USyncUsernameProtocol } from '../../WAUSync/Protocols/USyncUsernameProtocol.js';
import {} from '../../WAUSync/USyncQuery.js';
import { bridgeJidToString } from '../../Bridge/primitives.js';
import { jidDecode } from '../../WABinary/jid-utils.js';
const isUSyncMode = (value) => value === 'query' || value === 'full' || value === 'delta';
const isUSyncContext = (value) => value === 'interactive' || value === 'background' || value === 'message' || value === 'voip';
const isExactProtocol = (value, protocolClass) => Object.getPrototypeOf(value) === protocolClass.prototype;
const toNativeProtocol = (protocol) => {
    if (isExactProtocol(protocol, USyncContactProtocol)) {
        return { type: 'contact', data: { addressing_mode: 'pn' } };
    }
    if (isExactProtocol(protocol, USyncDeviceProtocol))
        return { type: 'devices' };
    if (isExactProtocol(protocol, USyncStatusProtocol))
        return { type: 'status' };
    if (isExactProtocol(protocol, USyncDisappearingModeProtocol))
        return { type: 'disappearing_mode' };
    if (isExactProtocol(protocol, USyncBotProfileProtocol))
        return { type: 'bot' };
    if (isExactProtocol(protocol, USyncLIDProtocol))
        return { type: 'lid' };
    if (isExactProtocol(protocol, USyncUsernameProtocol))
        return { type: 'username' };
    return undefined;
};
const supportedUSyncServers = new Set(['s.whatsapp.net', 'lid', 'hosted', 'hosted.lid', 'msgr', 'interop', 'bot']);
const toNativeJid = (value) => {
    const decoded = jidDecode(value);
    if (!decoded?.user)
        return undefined;
    const decodedServer = decoded.server;
    const server = decodedServer === 'c.us' ? 's.whatsapp.net' : decodedServer;
    if (!supportedUSyncServers.has(server))
        return undefined;
    return {
        user: decoded.user,
        server,
...
const toNativeUser = (user, selected) => {
    const native = {};
    ...
    if (selected.contact) {
        if (!user.phone && user.username) {
            ...
            if (user.usernameKey)
                native.username_pin = user.usernameKey;
        }
    ...
    if (user.lid && (selected.lid || contactUsesUsername)) {
        ...
        if (!lid || (lid.server !== 'lid' && lid.server !== 'hosted.lid'))
            return undefined;
        native.known_lid = lid;
    }
    if (selected.bot && user.personaId)
        native.persona_id = user.personaId;
    ...
    if (!native.id && !native.phone && !native.username)
        return undefined;
    return native;
};
/**
 ...
 */
export const toTypedUSyncQuery = (query) => {
    if (!isUSyncMode(query.mode) || !isUSyncContext(query.context) || query.users.length === 0)
        return undefined;
    const protocols = [];
    const protocolTypes = new Set();
    for (const protocol of query.protocols) {
        const native = toNativeProtocol(protocol);
        if (!native || protocolTypes.has(native.type))
            return undefined;
...
const throwSubprotocolError = (error) => {
    throw new Boom(error.text || 'Unknown error', {
        statusCode: error.code ?? 500,
        data: error.code
    });
};
const unwrapOutcome = (outcome) => {
    if (outcome.type === 'error')
        return throwSubprotocolError(outcome.data);
    return outcome.data;
};
const numberValue = (value) => Number(value);
const publicProtocolName = (result) => {
    switch (result.type) {
        case 'contact':
        case 'devices':
        case 'status':
...
const mapProtocolResult = (result, id, requested) => {
    const protocolName = publicProtocolName(result);
    if (!requested.has(protocolName))
        return undefined;
    switch (result.type) {
        case 'contact': {
        ...
        case 'devices': {
            ...
            const keyIndex = value.key_index
                ? {
                    ...
                    expectedTimestamp: value.key_index.expected_timestamp === null || value.key_index.expected_timestamp === undefined
                        ? undefined
...
export const fromTypedUSyncResponse = (query, response) => {
...
export const executeRawUSyncQuery = async (query, send) => query.parseUSyncQueryResult(await send(buildRawUSyncNode(query)));
