...
import { AsyncMutex, SerialQueue } from '../internal/async-primitives.js';
import { makeStandaloneCacheStore } from '../internal/cache-store.js';
...
export const addTransactionCapability = (state, logger, { maxCommitRetries, delayBetweenTriesMs }) => {
    ...
    const acquireTxMutexRef = (key) => txMutexRefCounts.set(key, (txMutexRefCounts.get(key) ?? 0) + 1);
    ...
    return {
        ...
        set: async (data) => {
            ...
            for (const key_ in data) {
                const key = key_;
                (_a = ctx.cache)[key] || (_a[key] = {});
                (_b = ctx.mutations)[key] || (_b[key] = {});
                if (key === 'pre-key') {
