 ...
 */
async function resolveSignalAddressId(id, lidMapping) {
    if (!id.includes('.'))
        return id;
...
export function makeLibSignalRepository(auth, logger, pnToLIDFunc) {
    ...
    const resolveLIDSignalAddress = (id) => resolveSignalAddressId(id, lidMapping);
    /**
     ...
     */
    const pinResolutionForStorage = (rawAddr, wireJid) => signalStorage(auth, lidMapping, async (id) => {
        if (id === rawAddr)
            return wireJid;
    ...
    // 🔐 Backup persistente de chaves em SQLite (dados/DB/chaves.db).
    // Singleton auto-inicializado; se better-sqlite3 não estiver instalado o
    // init falha silenciosamente e todos os métodos viram no-op.
    const keyBackupStore = getKeyBackupStore();
    keyBackupStore.init().then((ok) => {
        if (ok) {
            try {
                const stats = keyBackupStore.getStats?.();
                logger.info({ dbPath: stats?.dbPath }, '🔐 Key backup store inicializado');
            }
            catch { }
        }
    }).catch(() => { });
    // Constrói candidatos LID/PN para restaurar a sender-key certa do backup.
    const buildSenderKeyRestoreCandidates = async (group, authorJid, primaryId) => {
        const candidates = [primaryId];
        try {
            let altJid = null;
            if (isLidUser(authorJid)) {
                altJid = await lidMapping.getPNForLID(authorJid);
            }
            else if (isPnUser(authorJid)) {
                altJid = await lidMapping.getLIDForPN(authorJid);
            }
            if (altJid) {
                candidates.push(jidToSignalSenderKeyName(group, altJid).toString());
                try {
                    candidates.push(jidToSignalSenderKeyName(group, transferDevice(authorJid, altJid)).toString());
                }
                catch { }
            }
        }
    ...
    const normalizeSenderKeyBackup = (backup) => {
        // Formato novo: base64 (ver storeSenderKey).
        if (typeof backup === 'string') {
            return Buffer.from(backup, 'base64');
        }
        // Formato legado: objeto tipo Buffer serializado ({type:'Buffer',data:[...]}).
        if (backup && typeof backup === 'object' && backup.type === 'Buffer' && Array.isArray(backup.data)) {
            return Buffer.from(backup.data);
        }
        // Formato legado: Uint8Array serializado como {0:..,1:..}.
        if (backup && typeof backup === 'object' && !Buffer.isBuffer(backup) && !(backup instanceof Uint8Array)) {
            const ks = Object.keys(backup);
            if (ks.length > 0 && !isNaN(Number(ks[0]))) {
                const arr = new Uint8Array(ks.length);
    ...
    const repository = {
        decryptGroupMessage({ group, authorJid, msg }) {
            ...
            return parsedKeys.transactWith({ records: [{ type: 'sender-key', id: senderKeyId }] }, async () => {
                // Se a sender-key não existe no store primário, tenta restaurar
                // do backup SQLite antes de falhar (recuperação pós-restart).
                const { [senderKeyId]: existing } = await auth.keys.get('sender-key', [senderKeyId]);
                if (!existing) {
                    try {
                        const candidates = await buildSenderKeyRestoreCandidates(group, authorJid, senderKeyId);
                        for (const candidateId of candidates) {
                            const backup = await keyBackupStore.getSenderKeyBackup?.(candidateId);
                            if (backup) {
                                const data = normalizeSenderKeyBackup(backup);
                                const updates = { [senderKeyId]: data };
                                if (candidateId !== senderKeyId)
                                    updates[candidateId] = data;
                                await auth.keys.set({ 'sender-key': updates });
                                logger.info({ group, authorJid }, '✅ Sender key restaurada do backup SQLite');
                                break;
                            }
                        }
                    }
                    catch (err) {
                        logger.debug({ err: err?.message }, 'falha ao restaurar sender-key do backup');
                    }
                }
        ...
        async decryptMessage({ jid, type, ciphertext }) {
            ...
            // the cleared one.
            const records = [{ type: 'session', id: wireJid }];
            if (type === 'pkmsg') {
 ...
 */
pinnedResolver) {
    const resolveLIDSignalAddress = pinnedResolver ?? ((id) => resolveSignalAddressId(id, lidMapping));
    return {
        ...
        storeSenderKey: async (senderKeyName, key) => {
            ...
            await keys.set({ 'sender-key': { [keyId]: buf } });
            // Backup fire-and-forget para SQLite (recuperação pós-restart).
            // Guardado como base64 para round-trip sem ambiguidade de tipo.
            try {
                getKeyBackupStore().backupSenderKey?.(keyId, buf.toString('base64'));
            }
            catch { }
        },
