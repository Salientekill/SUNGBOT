...
const extractReportingTokenContent = (data, config) => {
    const wireVarint = 0;
    const wireBytes = 2;
    const fields = [];
    let i = 0;
...
export const getMessageReportingToken = async (msgProtobuf, message, key) => {
    ...
    const reportingSecret = await generateMsgSecretKey(ENC_SECRET_REPORT_TOKEN, key.id, from, to, message.messageContextInfo.messageSecret);
