    ...
    return {
        ...
        emit(event, evData) {
            if (isBuffering && BUFFERABLE_EVENT_SET.has(event)) {
                // 🔧 PR #2179: Fix message type overshadowing
                // If emitting messages.upsert and there are buffered messages with different type,
                // flush the buffer first to preserve the correct type for each batch
                if (event === 'messages.upsert') {
                    const { type } = evData;
                    const messageUpserts = Object.values(data.messageUpserts);
                    // Check if we have buffered messages with a different type
                    if (messageUpserts.length > 0 && messageUpserts[0].type !== type) {
                        logger.debug({ existingType: messageUpserts[0].type, newType: type }, 'flushing buffer due to message type mismatch');
                        flush();
                    }
                }
