...
const handlePrivacyTokensFromHistory = async (data, storePrivacyTokens, logger) => {
    if (data.privacyTokens?.length) {
        if (storePrivacyTokens) {
            ...
            logger?.debug({ count: data.privacyTokens.length }, 'skipped persisting privacy tokens: no storePrivacyTokens implementation');
        }
    }
};
const processAppStateSyncKeys = async (keys, keyStore, meId, logger) => {
...
const handlePeerDataOperationResponse = async (response, ev) => {
 ...
 */
export const getChatId = ({ remoteJid, participant, fromMe }, logger) => {
...
const processMessage = async (message, { shouldProcessHistoryMsg, placeholderResendCache, ev, creds, signalRepository, keyStore, logger, options, storePrivacyTokens }) => {
    const meId = creds.me.id;
    const { accountSettings } = creds;
    const chatId = getChatId(message.key, logger);
    if (!chatId) {
        logger?.error({ key: message.key }, 'could not determine chat ID from message key');
        return;
    }
    const chat = { id: jidNormalizedUser(chatId) };
