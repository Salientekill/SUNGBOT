...
const buildEditUpdate = (args) => {
    const editedInner = decryptWithJidFallback({ encPayload: args.encPayload, encIv: args.encIv }, args.editEncKey, args.originalMsgId, args.primary, args.fallback);
 ...
 * both attempts fail.
 */
function decryptWithJidFallback(encrypted, editEncKey, originalMsgId, primary, fallback) {
    try {
        ...
        try {
            ...
            throw new Error(`edit decrypt failed: primary=${primaryErr.message}; fallback=${fallbackErr.message}`);
        }
    }
}
/**
...
const processMessage = async (message, { shouldProcessHistoryMsg, placeholderResendCache, processedMessageCache, ev, creds, signalRepository, keyStore, logger, options, getMessage, getMediaHost }) => {
    ...
    if (processedMessageCache && message.key?.id) {
        ...
        const alreadyProcessed = await processedMessageLocks.withLock({ namespace: 'process-msg', id: idempotencyKey }, async () => {
            const already = await processedMessageCache.get(idempotencyKey);
            if (already)
                return true;
    ...
    return (async () => {
        ...
        if (protocolMsg) {
            ...
            switch (protocolMsg.type) {
                case proto.Message.ProtocolMessage.Type.HISTORY_SYNC_NOTIFICATION:
                    ...
                    if (process) {
                        ...
                        const data = await downloadAndProcessHistorySyncNotification(histNotification, options, logger, getMediaHost?.());
            ...
            if (!targetKey?.id || !secEnc.encPayload?.length || secEnc.encIv?.length !== 12) {
                logger?.warn({ targetKey, ivLen: secEnc.encIv?.length, hasPayload: !!secEnc.encPayload?.length }, 'MESSAGE_EDIT envelope malformed (missing targetKey/payload or IV != 12) — skipping');
            }
            else {
                ...
                if (!targetMsg) {
                    ...
                    try {
                        ...
                        if (!editEncKey) {
                            ...
                            const origSenderRaw = targetKey.participant || (targetKey.fromMe ? envelopeAuthorRaw : targetKey.remoteJid);
                            ...
                            const fallback = altOrig || altEditor
                                ? {
