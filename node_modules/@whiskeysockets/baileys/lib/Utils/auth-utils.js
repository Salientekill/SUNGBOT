...
/**
 * Retorna o TTL apropriado para cada tipo de chave
 */
function getTTLForType(type) {
    switch (type) {
        case 'session':
            return DEFAULT_CACHE_TTLS.SESSION || 2 * 60 * 60; // 2 horas
        case 'sender-key':
        case 'sender-key-memory':
            return DEFAULT_CACHE_TTLS.SENDER_KEY || 2 * 60 * 60; // 2 horas
        case 'pre-key':
        case 'signed-pre-key':
            return DEFAULT_CACHE_TTLS.PRE_KEY || 30 * 60; // 30 min
        case 'app-state-sync-key':
        case 'app-state-sync-version':
            return DEFAULT_CACHE_TTLS.APP_STATE || 60 * 60; // 1 hora
        default:
            return DEFAULT_CACHE_TTLS.SIGNAL_STORE || 60 * 60; // 1 hora padrão
    }
}
/**
 * Adds caching capability to a SignalKeyStore
 * 🚀 VERSÃO MELHORADA com TTLs específicos por tipo de chave
...
export function makeCacheableSignalKeyStore(store, logger, _cache) {
    const cache = _cache ||
        new NodeCache({
            stdTTL: DEFAULT_CACHE_TTLS.SIGNAL_STORE, // TTL padrão
            useClones: false,
            deleteOnExpire: true,
        ...
        async get(type, ids) {
            return cacheMutex.runExclusive(async () => {
                ...
                if (idsToFetch.length) {
                    logger?.trace({ items: idsToFetch.length, type, hits: cacheHits, misses: cacheMisses }, 'loading from store');
                    ...
                    for (const id of idsToFetch) {
                        ...
                        if (item) {
                            data[id] = item;
                            // Usar TTL específico para este tipo de chave
                            cache.set(getUniqueId(type, id), item, ttl);
                        }
                    }
                }
                return data;
            ...
            return cacheMutex.runExclusive(async () => {
                ...
                for (const type in data) {
                    ...
                    for (const id in data[type]) {
                        ...
                        // Se valor é null, remover do cache
                        if (value === null) {
                            cache.del(getUniqueId(type, id));
                        }
                        else {
                            await cache.set(getUniqueId(type, id), value, ttl);
                        }
                        keys += 1;
                    }
                }
