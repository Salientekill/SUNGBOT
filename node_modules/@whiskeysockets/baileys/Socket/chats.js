...
import { chatModificationToAppPatch, decodePatches, decodeSyncdSnapshot, encodeSyncdPatch, ensureLTHashStateVersion, extractSyncdPatches, generateProfilePicture, getHistoryMsg, newLTHashState, processSyncAction } from '../Utils/index.js';
import { makeMutex } from '../Utils/make-mutex.js';
import processMessage from '../Utils/process-message.js';
import { BlockedCollectionsManager, classifySyncError, prepareCollectionSyncNode, SyncErrorAction } from '../Utils/sync-action-utils.js';
...
export const makeChatsSocket = (config) => {
                    ...
                    try {
                        ...
                        switch (classification.action) {
                            case SyncErrorAction.BLOCK_ON_KEY:
                                ...
                                logger.info({ name, error: classification.errorMessage }, 'collection blocked waiting for app-state-sync-key');
                                blockedCollections.block(name);
                                collectionsToHandle.delete(name);
                                break;
                            case SyncErrorAction.RETRY:
                                // Retriable error - reset state and try again
                                logger.info({ name, error: classification.errorStack ?? classification.errorMessage }, 'failed to sync state from version, removing and trying from scratch');
                                await authState.keys.set({ 'app-state-sync-version': { [name]: null } });
                                attemptsMap[name] = (attemptsMap[name] ?? 0) + 1;
                                break;
                            case SyncErrorAction.ABORT:
                                // Irrecoverable error - stop trying
                                logger.info({ name, error: classification.errorStack ?? classification.errorMessage }, 'failed to sync state from version');
                                await authState.keys.set({ 'app-state-sync-version': { [name]: null } });
                                collectionsToHandle.delete(name);
                                break;
                        }
    ...
    ev.on('creds.update', async (update) => {
        if (update.myAppStateKeyId && blockedCollections.hasBlocked) {
            ...
            logger.info({ collections: collectionsToRetry, newKeyId: update.myAppStateKeyId }, 'app-state-sync-key arrived, retrying blocked collections');
            try {
