...
import { isTcTokenExpired, resolveTcTokenJid } from '../Utils/tc-token-utils.js';
...
export const makeMessagesRecvSocket = (config) => {
    ...
    const { ev, authState, ws, messageMutex, notificationMutex, receiptMutex, signalRepository, query, upsertMessage, resyncAppState, onUnexpectedError, assertSessions, sendNode, relayMessage, sendReceipt, uploadPreKeys, sendPeerDataOperationMessage, messageRetryManager, registerSocketEndHandler, getPrivacyTokens } = sock;
        ...
        if (from === S_WHATSAPP_NET) {
            ...
            if (result.action === 'session_refreshed') {
                ...
                try {
                    ...
                    const tcJid = await resolveTcTokenJid(normalizedJid, signalRepository.lidMapping.getLIDForPN.bind(signalRepository.lidMapping));
                    ...
                    if (senderTs !== null && senderTs !== undefined && !isTcTokenExpired(senderTs)) {
                        ...
                        getPrivacyTokens([normalizedJid], senderTs).catch(err => {
                            logger.debug({ jid: normalizedJid, err: err?.message }, 'failed to re-issue tctoken after identity change');
                        });
                    }
                }
                catch (err) {
    ...
    const tcTokenRetriedMsgIds = new Set();
    ...
    const sendMessagesAgain = async (key, ids, retryNode) => {
        ...
        for (const id of ids) {
            ...
            if (messageRetryManager) {
                ...
                if (cachedMsg) {
                    // Safety check: never resend protocol messages (edits, deletes, reactions)
                    // These share the same message ID as the original and should never be relayed
                    const innerContent = normalizeMessageContent(cachedMsg.message);
                    const isProtocolMsg = !!(innerContent?.protocolMessage ||
                        innerContent?.reactionMessage ||
                        innerContent?.senderKeyDistributionMessage);
                    if (isProtocolMsg) {
                        logger.debug({ jid: remoteJid, id }, 'skipping protocol message in retry cache');
                    }
                    else {
            ...
            if (!msg) {
                ...
                if (msg) {
                    // Same safety check for getMessage results
                    const innerContent = normalizeMessageContent(msg);
                    const isProtocolMsg = !!(innerContent?.protocolMessage ||
                        innerContent?.reactionMessage ||
                        innerContent?.senderKeyDistributionMessage);
                    if (isProtocolMsg) {
                        logger.debug({ jid: remoteJid, id }, 'skipping protocol message from getMessage');
                        msg = undefined;
                    }
                    else {
        ...
        // When a retry comes from a primary JID (no device number), the original
        // code would broadcast to ALL group members (sendToAll = true).
        // This causes the message to appear as a NEW message for everyone in the group,
        // which is the root cause of phantom re-sends. Instead, always target only
        // the specific participant that requested the retry.
    ...
    const handleMessage = async (node) => {
        ...
        try {
            ...
            await messageMutex.mutex(async () => {
                await decrypt();
                // Only cache INCOMING regular messages for retry — skip:
                // 1. Protocol messages (edits, deletes/revokes, reactions) as they share
                //    the same ID as the original and would overwrite the cache
                // 2. Messages fromMe (bot's own echoed back) — already cached by relayMessage,
                //    re-caching here would create a double-send loop
                if (msg.key?.remoteJid && msg.key?.id && msg.message && messageRetryManager && !msg.key.fromMe) {
                    const innerContent = normalizeMessageContent(msg.message);
                    const isProtocolMsg = !!(innerContent?.protocolMessage ||
                        innerContent?.reactionMessage ||
                        innerContent?.senderKeyDistributionMessage);
                    if (!isProtocolMsg) {
        ...
        if (attrs.error) {
            if (attrs.error === SERVER_ERROR_CODES.MissingTcToken) {
                ...
                if (msgId && jid && !tcTokenRetriedMsgIds.has(msgId)) {
                    ...
                    setTimeout(() => tcTokenRetriedMsgIds.delete(msgId), 60000);
                    const msg = await getMessage(key);
                    if (msg) {
    ...
    // When a message is edited or deleted, remove it from the retry cache.
    // This prevents the retry mechanism from resending an outdated or revoked
    // message when a device sends a retry receipt.
    if (messageRetryManager) {
        ev.on('messages.update', updates => {
            for (const { key, update } of updates) {
                ...
                // Message was revoked/deleted (message set to null with REVOKE stub)
                if (update.message === null && update.messageStubType === WAMessageStubType.REVOKE) {
                    messageRetryManager.invalidateMessage(key.id);
                    logger.debug({ msgId: key.id }, 'cleared retry cache for revoked message');
                }
                // Message was edited (contains editedMessage wrapper)
                if (update.message?.editedMessage) {
                    messageRetryManager.invalidateMessage(key.id);
                    logger.debug({ msgId: key.id }, 'cleared retry cache for edited message');
                }
            }
        });
    }
