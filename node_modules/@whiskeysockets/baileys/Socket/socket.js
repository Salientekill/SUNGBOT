...
export const makeSocket = (config) => {
     ...
     * setInterval to prevent overlapping pings. Sends fire-and-forget pings
     * via sendNode (not query) to avoid blocking the timer loop. Liveness is
     * determined by lastDateRecv (updated on ANY server data). Tracks
     * consecutive SEND failures separately; after MAX_PING_FAILURES the
    ...
    const startKeepAliveRequest = () => {
        const scheduleNextPing = () => {
            keepAliveReq = setTimeout(async () => {
                ...
                /*
                 * Hard timeout: if no data received from server for more than
                 * 2 keepalive intervals + grace period, connection is dead.
                 * With fire-and-forget pings, the timer fires on schedule,
                 * so this check runs reliably every keepAliveIntervalMs.
                 */
    ...
    // Handle server-initiated pings — critical for connection stability.
    // The WA server sends pings to the client in two formats and expects a result response.
    // Without responding, the server stops sending data → lastDateRecv freezes → 408 timeout.
    //
    // Format 1: <iq type="get"><ping/></iq> or <iq type="get" xmlns="w:p"><ping/></iq>
    ws.on('CB:iq,type:get,ping', async (stanza) => {
        await sendNode({
            tag: 'iq',
            attrs: {
                to: stanza.attrs.from || S_WHATSAPP_NET,
                type: 'result',
                id: stanza.attrs.id
            }
        }).catch(err => logger.warn({ err }, 'failed to respond to server ping'));
    });
    // Format 2: <iq type="get" xmlns="urn:xmpp:ping"/> (XMPP standard, no child element)
    ws.on('CB:iq,xmlns:urn:xmpp:ping', async (stanza) => {
