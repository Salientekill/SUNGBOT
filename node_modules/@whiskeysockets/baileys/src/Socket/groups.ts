...
export const makeGroupsSocket = (config: SocketConfig) => {
	...
	sock.ws.on('CB:ib,,dirty', async (node: BinaryNode) => {
		...
		// Verificação defensiva: retorna se dirty não existir
		if (!dirty) {
			return
		}
		const { attrs } = dirty
...
export const extractGroupMetadata = (result: BinaryNode) => {
	...
	// Verificação defensiva: se não houver group node, lançar erro claro
	if (!group) {
	...
	if (descChild) {
		...
		// Usar || 0 para evitar NaN se t for undefined
	...
	const metadata: GroupMetadata = {
		...
		memberAddMode,
		participants: getBinaryNodeChildren(group, 'participant').map((node) => {
			const { attrs } = node
			// TODO: Store LID MAPPINGS
			
			// Extrair member_label se existir
			const memberLabelNode = getBinaryNodeChild(node, 'member_label')
			let memberLabel: GroupParticipant['memberLabel'] = undefined
			if (memberLabelNode) {
				const labelContent = memberLabelNode.content
				const labelText = typeof labelContent === 'string' 
					? labelContent 
					: Buffer.isBuffer(labelContent) 
						? labelContent.toString('utf-8')
						: undefined
				if (labelText) {
					memberLabel = {
						label: labelText,
						labelTimestamp: memberLabelNode.attrs.t ? +memberLabelNode.attrs.t : undefined
					}
				}
			}
