...
import { markParticipantForKey, startWarmup } from '../Utils/priority-participants.js'
...
export const makeChatsSocket = (config: SocketConfig) => {
	...
	const handlePresenceUpdate = ({ tag, attrs, content }: BinaryNode) => {
		...
		if (presence) {
			...
			// 🔐 Se é presença em grupo, marcar participante para receber chave
			// Isso resolve "Aguardando mensagem" para quem está online no grupo
			if (jid?.endsWith('@g.us') && participant && participant !== jid) {
				markParticipantForKey(jid, participant)
			}
		}
	}
	...
	ev.on('connection.update', ({ connection, receivedPendingNotifications }) => {
		if (connection === 'open') {
			...
			// 🔥 Iniciar modo de aquecimento após conexão
			// Distribui chaves gradualmente para evitar "Aguardando mensagem"
			startWarmup()
		}
