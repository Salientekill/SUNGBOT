...
export class GroupCipher {
	...
	private getSenderKey(senderKeyState: SenderKeyState, iteration: number) {
		...
		// Reduzido de 2000 para 500 porque:
		// 1. 2000 iterações bloqueiam event loop por muito tempo
		// 2. Gap grande = sender key provavelmente está obsoleta
		// 3. Melhor solicitar nova SKDM do que iterar 2000x
		const MAX_ITERATION_GAP = 500
		if (iteration - senderChainKey.getIteration() > MAX_ITERATION_GAP) {
			throw new Error(`Over ${MAX_ITERATION_GAP} messages into the future - need fresh SKDM`)
		}
